...
class AudioManagerClass
{
    ...
    public:
         ...
         */
        virtual AudioHandleID Request_Play(Wstring filename, AudioGroupType group, float volume = 1.0f, float pitch = 1.0f, float pan = 0.0f, AudioPriorityType priority = AUDIO_PRIORITY_NORMAL, int limit = -1, float fade_in_seconds = 0.0f, float delay_in_seconds = 0.0f, bool start = true, bool looping = false);
        ...
        virtual bool Query_Sample_Ready(Wstring name, AudioGroupType group);
         ...
         */
        virtual bool Submit_Sample(Wstring &filename, AudioFileType filetype, AudioGroupType group, AudioPriorityType priority, AudioControlType control, AudioSoundType type, unsigned int limit);
        virtual bool Clear_Samples(AudioGroupType group = AUDIO_GROUP_NONE);
        virtual bool Has_Been_Submitted(Wstring &filename, AudioGroupType group = AUDIO_GROUP_NONE);

        bool Lock_Submissions() { SubmissionsLocked = true; }
        ...
        virtual bool Is_File_Available(Wstring name) const;
        bool Is_FileType_Supported(AudioFileType type) const;
        Wstring Build_Filename_From_Type(AudioFileType type, Wstring name);

        bool Get_File_Info(Wstring name, AudioFileType &filetype, Wstring &filename, bool ignore_error = false);
         ...
         */
        bool Open_VQ_Audio_Stream(const Wstring& name, int sampleRate, int channels, int bitsPerSample, float volume = 1.0f);
    ...
    private:
        ...
        AudioSampleClass * Find_Sample(const Wstring & filename, AudioGroupType group);

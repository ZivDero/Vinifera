 ...
 *  @author: CCHyper
 */
bool AudioInstanceClass::Load(/*Wstring filename, ma_sound_group *group*/)
{
    ma_result result;
    ...
    if (Audio_IsAUDFile(Template->Get_FileName())) {
        ...
        result = ma_decoder_init_vfs(&ma_custom_vfs_callbacks, Template->Get_FileName().Peek_Buffer(), &decoderConfig, Decoder);
        ...
        result = ma_sound_init_from_file(AudioManager.Engine, Template->Get_FileName().Peek_Buffer(), flags, AudioManager.SoundGroups[Template->Get_Group()], nullptr, Sound);
        if (result != MA_SUCCESS) {

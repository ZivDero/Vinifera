...
// NOTE (Vinifera): SDL3_image dependency stripped. LoadTexture is stubbed to return 0
// because Vinifera's RmlUi integration does not load images from disk via the file
// iface. Image data reaches the renderer only through GenerateTexture (raw bytes
// produced by RmlUi's font rasterizer or CSS decorators).
...
void RenderInterface_SDL_GPU::ReleaseGeometryCommand::Update(RenderInterface_SDL_GPU& iface)
{
	(void)iface;
...
void RenderInterface_SDL_GPU::RenderGeometryCommand::Update(RenderInterface_SDL_GPU& iface)
{
	if (!iface.BeginRenderPass())
	{
		return;
	}
	...
	if (texture != 0)
	{
		SDL_BindGPUGraphicsPipeline(iface.render_pass, iface.texture_pipeline);

		SDL_GPUTextureSamplerBinding texture_binding{};
		texture_binding.texture = reinterpret_cast<SDL_GPUTexture*>(texture);
		texture_binding.sampler = iface.linear_sampler;
		SDL_BindGPUFragmentSamplers(iface.render_pass, 0, &texture_binding, 1);
	}
	else
	{
		SDL_BindGPUGraphicsPipeline(iface.render_pass, iface.color_pipeline);
	}
	...
	SDL_BindGPUVertexBuffers(iface.render_pass, 0, &vertex_buffer_binding, 1);
	SDL_BindGPUIndexBuffer(iface.render_pass, &index_buffer_binding, SDL_GPU_INDEXELEMENTSIZE_32BIT);

	SDL_SetGPUScissor(iface.render_pass, &iface.scissor);

	SDL_PushGPUVertexUniformData(iface.command_buffer, 0, &iface.transform, sizeof(iface.transform));
	SDL_PushGPUVertexUniformData(iface.command_buffer, 1, &translation, sizeof(translation));

	SDL_DrawGPUIndexedPrimitives(iface.render_pass, geometry->num_indices, 1, 0, 0, 0);
}
...
void RenderInterface_SDL_GPU::EnableScissorRegionCommand::Update(RenderInterface_SDL_GPU& iface)
{
	if (!enable)
	{
		iface.scissor.x = 0;
		iface.scissor.y = 0;
		iface.scissor.w = iface.swapchain_width;
		iface.scissor.h = iface.swapchain_height;
	}
}
...
void RenderInterface_SDL_GPU::SetScissorRegionCommand::Update(RenderInterface_SDL_GPU& iface)
{
	iface.scissor.x = region.Left();
	iface.scissor.w = region.Width();
	iface.scissor.y = region.Top();
	iface.scissor.h = region.Height();
}
...
{
	// NOTE (Vinifera): file-backed image loading is not used by the Vinifera RmlUi
	// integration; documents only need raw glyph/atlas data which arrives via
	// GenerateTexture. If image-decorator support is needed later, route through
	// the FileInterface and add a PNG/TGA decoder of choice here.
	return 0;
}
...
void RenderInterface_SDL_GPU::ReleaseTextureCommand::Update(RenderInterface_SDL_GPU& iface)
{
	SDL_GPUTexture* texture = reinterpret_cast<SDL_GPUTexture*>(handle);
	SDL_ReleaseGPUTexture(iface.device, texture);
}
...
void RenderInterface_SDL_GPU::SetTransformCommand::Update(RenderInterface_SDL_GPU& iface)
{
	if (has_transform)
	{
		iface.transform = iface.proj * transform;
	}
	else
	{
		iface.transform = iface.proj;
	}
}
